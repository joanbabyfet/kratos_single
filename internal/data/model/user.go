...
type User struct {
	...
	Origin       int8   `gorm:"default(0);null;description(注册来源 1=H5 2=PC)"`
	...
	Avatar       string `gorm:"size(100);default();null;description(头像)"`
	...
	PhoneCode    string `gorm:"size(5);default();null;index;description(手机号国码)"`
	Phone        string `gorm:"unique;size(20);default();null;index;description(手机号)"`
	Address      string `gorm:"size(100);default();null;description(地址)"`
	...
	Language     string `gorm:"size(10);default();null;description(语言)"`
